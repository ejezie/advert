...
exports.create = function(req, res) {
	...
	comment.postAdvert = req.postAdvert;
	...
	comment.save(function(err) {
		if (err) {
			return res.status(400).send({
				...
				req.postAdvert.comments.push(comment);
				req.postAdvert.save(function(err){
					if(!err)
						res.jsonp(comment);
				});
...
exports.list = function(req, res) { Comment.find({postAdvert:req.postAdvert}).sort('-created').populate('user', 'displayName').exec(function(err, comments) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
